...
'
        ...
        log_result "Warning: SNAPSHOT rootfs uses OPKG but gh-pages has APK packages"
        log_result "This test will fail until rootfs is updated or we build OPKG packages"
        TEST_SCRIPT='
set -e
mkdir -p /var/lock
...
if wget -q --spider https://ly4096x.github.io/my-openwrt-packages/SNAPSHOT/packages/x86_64/ly4096x-packages-feeds_20250103-r1_all.ipk; then
    ...
    echo "ERROR: SNAPSHOT packages are in APK format but rootfs uses OPKG"
    exit 1
fi
'
    fi
